...
# Graded diabetic-retinopathy keywords -> ICDR-style grade, most specific first (a
# token is matched by substring, so "severe proliferative diabetic retinopathy" and
# "suspected moderate non proliferative retinopathy" land on their grade). ODIR
# spells "non proliferative" both with and without the space.
_ODIR_DR_GRADES = (("proliferative diabetic retinopathy", 4.0),
                   ("severe nonproliferative retinopathy", 3.0), ("severe non proliferative retinopathy", 3.0),
...
def _odir_tokens(keywords) -> List[str]:
    """ODIR's keyword string (full-width or ASCII commas) -> lower-cased tokens."""
    if keywords is None or keywords != keywords:
        return []
    return [t.strip().lower() for t in re.split(r"[,\uff0c;]", str(keywords)) if t.strip()]
...
def odir_age_fields(keywords) -> Dict[str, float]:
    """Per-eye fields for the retinal-age cohort rule, read from the diagnostic keywords.

    ``final_quality``  1.0 unless a quality remark (``_LOW_QUALITY``) is present.
    ``normal_fundus``  1.0 when the only finding is "normal fundus"; 0.0 when any
                       other finding is listed; NaN when there is no finding at all
                       (empty, or quality remarks only) -- never a silent "normal".
    ``final_icdr``     DR grade 0-4 from the graded retinopathy keywords; 0.0 when
                       findings are listed and none is diabetic retinopathy; NaN when
                       DR is named without a grade, or there is no finding to read.
    Verified against the Kaggle full_df.csv (6,392 eyes, 94 distinct keywords).
    """
    toks = _odir_tokens(keywords)
    is_q = lambda t: any(q in t for q in _LOW_QUALITY)
    quality = 0.0 if any(is_q(t) for t in toks) else 1.0
    findings = [t for t in toks if not is_q(t)]
    if not findings:
        return {"final_quality": quality, "normal_fundus": np.nan, "final_icdr": np.nan}
    normal = 1.0 if all(t == "normal fundus" for t in findings) else 0.0
    grade, ungraded = 0.0, False
    for t in findings:
        g = next((v for k, v in _ODIR_DR_GRADES if k in t), None)
        if g is not None:
            grade = max(grade, g)
        elif _ODIR_DR_UNGRADED in t:
            ungraded = True
    if ungraded and grade == 0.0:
        grade = np.nan
    return {"final_quality": quality, "normal_fundus": normal, "final_icdr": grade}
...
def load_odir(root: str, image_ext: str = ".jpg") -> Dict[str, object]:
    ...
    for _, r in t.iterrows():
        ...
        for fn, side in eyes:
            kw = r[lkw] if side == "left" else r[rkw]
            g, a = _odir_eye_labels(kw)
            ...
            rows.append({"file": fn, "patient": pid if pid else os.path.splitext(fn)[0].split("_")[0],
                         ...
                         "keywords": (str(kw).strip().lower() if kw == kw and kw is not None else ""),
                         "camera": "odir", **odir_age_fields(kw)})
    df = pd.DataFrame(rows)
